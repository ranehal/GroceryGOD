...
nb["cells"][0]["source"] = [
    ...
    "- **🔑 Pre-Flight Secrets Fail-Fast & Expanded Fallback Discovery (2026-09-04)**: Added immediate fail-fast termination (`sys.exit(1)`) when core secrets are missing or invalid, preventing 10-minute wasted sleep and cascading child process crashes. Enhanced secret resolution to scan all attached Kaggle dataset JSONs (`/kaggle/input/**/*.json`) with key aliasing and inspect local git extraheader/remotes for automatic PAT discovery.\n",
