...
def scrape_unimart():
    ...
    for query in search_queries:
        ...
        while True:
            if offset >= 10000:
                logger.warning(f"  [!] Max offset reached for query '{query}', stopping.")
                break
            ...
            try:
                ...
                if connection_failures >= 3:
                    logger.error("Repeated connection failures — aborting Unimart scrape (API unreachable).")
                    save_data(data)
                    save_last_run_log(summary)
                    return summary['total'], summary['new']
                time.sleep(5)
                continue
