...
function loadDemoData() {
    ...
    const storeFactor = { shwapno: 1.00 };
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        document.title = "Shwapno Price Monitor";
        showLoading(true, 'Initializing Shwapno Matrix...');
...
async function loadAllFromJson() {
    let rawData = [];
    if (window.shwapno_data && Array.isArray(window.shwapno_data)) {
        rawData = window.shwapno_data;
    } else if (window.shwapno_products && Array.isArray(window.shwapno_products)) {
        rawData = window.shwapno_products;
    } else {
        try {
            const resp = await fetch('shwapno_products.json');
            if (resp.ok) rawData = await resp.json();
        } catch (e) {
            console.error('Failed to load shwapno_products.json:', e);
        }
    }
    ...
    allProducts = rawData.map(item => {
        let store = 'shwapno';
        let cp = (item.category_path || item.category || 'General').replace(/^>\s*/, '').trim();
        let parts = cp.split('>').map(s => s.trim()).filter(Boolean);
        
        let catParent = parts.length >= 1 ? parts[0] : 'General';
        let catChild = parts.length >= 2 ? parts.slice(1).join(' > ') : (item.category || catParent);

        let normPrice = item.normalized_price || item.current_price || item.display_price || 0;
        let currPrice = item.current_price || item.display_price || normPrice;
        let oldPrice = item.old_price != null ? parseFloat(item.old_price) : null;

        let history = [];
        if (Array.isArray(item.history) && item.history.length > 0) {
            history = item.history;
        } else if (item.price_history && typeof item.price_history === 'object') {
            history = Object.entries(item.price_history).map(([d, p]) => ({
                date: d,
                price: parseFloat(p),
                normalized_price: parseFloat(p)
            }));
        }

        if (oldPrice && oldPrice > currPrice && history.length <= 1) {
            let prevDate = item.first_seen || '2026-07-30';
            let currDate = item.last_seen || today;
            ...
            history = [
                { date: prevDate, price: oldPrice, normalized_price: oldPrice },
                { date: currDate, price: currPrice, normalized_price: normPrice }
            ];
        }

        if (history.length === 0) {
            let date = item.last_seen || item.first_seen || today;
            history = [{ date, price: currPrice, normalized_price: normPrice }];
        }

        let prices = history.map(h => h.normalized_price || h.price);
        let minPrice = Math.min(...prices);
        let maxPrice = Math.max(...prices);
        let avgPrice = prices.reduce((a, b) => a + b, 0) / prices.length;

        let dropPct = 0;
        if (oldPrice && oldPrice > 0 && currPrice < oldPrice) {
            dropPct = -((oldPrice - currPrice) / oldPrice) * 100;
        } else if (prices.length > 1 && prices[0] > 0) {
            dropPct = ((prices[prices.length - 1] - prices[0]) / prices[0]) * 100;
        }

        let discountText = item.discount_text || (oldPrice && oldPrice > currPrice ? (Math.round(((oldPrice - currPrice)/oldPrice)*100) + '% OFF') : '');

        return {
            id: item.id || ('shwapno_' + Math.random().toString(36).substr(2, 9)),
            name: item.name,
            store: 'shwapno',
            ...
            display_price: currPrice,
            old_price: oldPrice,
            discount_text: discountText,
            image: item.image || '',
            url: item.url ? (item.url.startsWith('http') ? item.url : ('https://www.shwapno.com/p/' + item.url)) : '#',
            rating: item.rating || 0,
            total_reviews: item.total_reviews || 0,
            is_best_seller: item.is_best_seller || false,
            is_new: item.is_new || false,
            stock: item.stock || 'InStock',
            first_seen: item.first_seen || history[0].date,
            last_seen: item.last_seen || history[history.length - 1].date,
            history: history,
            hist_count: history.length,
            minPrice: minPrice,
            maxPrice: maxPrice,
            avgPrice: avgPrice,
            oldest_date: history[0].date,
            newest_date: history[history.length - 1].date,
            priceChangePercent: dropPct,
            _historyLoaded: true
        };
    });
    ...
    metadata.stores = {
        shwapno: {
            ...
            date_range: (allProducts[0]?.oldest_date || '2026-07-30') + ' to ' + today
        }
    };
    window.loadedStores = new Set(['shwapno']);
    activeShopFilters = new Set(['shwapno']);
    activeCategories = new Set();
    allProducts.forEach(p => activeCategories.add('shwapno_' + p.category));
}
...
function renderSidebar() {
    ...
    shopHeading.innerHTML = '<span><i class="fas fa-microchip"></i> Product Categories</span>';
    list.appendChild(shopHeading);

    Object.keys(STORE_CONFIG).forEach(sid => {
        const shopProducts = allProducts.filter(p => p.store === sid);
        const isShopActive = activeShopFilters.has(sid);

        const group = document.createElement('div'); group.className = 'shop-group';
        const header = document.createElement('div');
        header.dataset.sid = sid;
        header.className = 'shop-header ' + (isShopActive ? 'active' : '');
        header.innerHTML = `
            <div class="shop-toggle-container">
                <input type="checkbox" class="shop-checkbox" ${isShopActive ? 'checked' : ''}>
                <span style="color:${STORE_CONFIG[sid].color}; font-weight:800;">${STORE_CONFIG[sid].name}</span>
            </div>
            <div style="display:flex; align-items:center; gap:12px;">
                <span style="opacity:0.6; font-size:0.75rem;">${shopProducts.length.toLocaleString()} units</span>
                <i class="fas fa-chevron-down toggle-icon" style="font-size:0.7rem; padding: 6px;"></i>
            </div>
        `;
        ...
        header.onclick = (e) => {
            ...
            if (cb.checked) {
                ...
                shopProducts.forEach(p => activeCategories.add(sid + '_' + p.category));
            } else {
                activeShopFilters.delete(sid);
                shopProducts.forEach(p => activeCategories.delete(sid + '_' + p.category));
            }
            renderSidebar();
            renderProducts();
            updateStatsBar();
        ...
        shopProducts.forEach(p => {
            const parent = p.category_parent || 'General';
            const child = p.category || 'General';
            if (!parentMap[parent]) parentMap[parent] = {};
            if (!parentMap[parent][child]) parentMap[parent][child] = [];
            parentMap[parent][child].push(p);
        });

        const sortedParents = Object.keys(parentMap).sort((a, b) => a.localeCompare(b));

        sortedParents.forEach(parentName => {
            const subMap = parentMap[parentName];
            ...
            subNames.forEach(sub => {
                const count = subMap[sub].length;
                parentTotalCount += count;
                if (activeCategories.has(sid + '_' + sub)) {
                    parentActiveCount += 1;
                }
            });
            ...
            parentGroup.dataset.parentName = parentName;

            const isParentAllActive = parentActiveCount === subNames.length && subNames.length > 0;
            const isParentPartial = parentActiveCount > 0 && parentActiveCount < subNames.length;

            const parentItem = document.createElement('div');
            parentItem.className = `parent-cat-item ${isParentAllActive || isParentPartial ? 'active' : ''}`;
            parentItem.innerHTML = `
                <div style="display:flex; align-items:center; gap:8px; flex:1;">
                    <input type="checkbox" class="parent-checkbox" ${isParentAllActive ? 'checked' : ''}>
                    <span class="parent-name">${parentName}</span>
                </div>
                <div style="display:flex; align-items:center; gap:8px;">
                    <span style="opacity:0.5; font-size:0.7rem;">${parentTotalCount}</span>
                    <button type="button" class="parent-toggle-btn expanded" title="Toggle subcategories"><i class="fas fa-chevron-down"></i></button>
                </div>
            `;

            const parentCb = parentItem.querySelector('.parent-checkbox');
            if (isParentPartial) parentCb.indeterminate = true;

            const toggleBtn = parentItem.querySelector('.parent-toggle-btn');
            const subList = document.createElement('ul');
            subList.className = 'sub-cat-list expanded';

            parentItem.onclick = (e) => {
                if (e.target.closest('.parent-toggle-btn')) {
                    const isExp = subList.classList.contains('expanded');
                    subList.classList.toggle('expanded', !isExp);
                    toggleBtn.classList.toggle('expanded', !isExp);
                    return;
                }
                if (e.target !== parentCb) parentCb.checked = !parentCb.checked;
                if (parentCb.checked) {
                    subNames.forEach(sub => activeCategories.add(sid + '_' + sub));
                    activeShopFilters.add(sid);
                } else {
                    subNames.forEach(sub => activeCategories.delete(sid + '_' + sub));
                    const anyShopLeft = shopProducts.some(p => activeCategories.has(sid + '_' + p.category));
                    if (!anyShopLeft) activeShopFilters.delete(sid);
                }
                renderSidebar();
                renderProducts();
                updateStatsBar();
            ...
            subNames.forEach(subName => {
                const subProducts = subMap[subName];
                const count = subProducts.length;
                const newCount = subProducts.filter(p => p.ageDays <= 7 || p.is_new).length;
                const catId = sid + '_' + subName;
                const isCatActive = activeCategories.has(catId);

                const li = document.createElement('li');
                li.className = `shop-cat-item ${isCatActive ? 'active' : ''}`;
                li.dataset.catName = subName;
                li.innerHTML = `
                    <div class="cat-row-content" style="display:flex; align-items:center; gap:8px; flex:1;">
                        <input type="checkbox" class="cat-checkbox" ${isCatActive ? 'checked' : ''}>
                        <span class="cat-name">${subName}</span>
                    </div>
                    <div style="display:flex; align-items:center; gap:6px;">
                        ${newCount > 0 ? '<span class="new-tag-tiny" title="New items">+ ' + newCount + '</span>' : ''}
                ...
                li.onclick = (e) => {
                    ...
                    if (catCb.checked) {
                        ...
                        const anyLeft = shopProducts.some(p => activeCategories.has(sid + '_' + p.category));
                        if (!anyLeft) activeShopFilters.delete(sid);
                    }
                    renderSidebar();
                    renderProducts();
                    updateStatsBar();
            ...
            parentGroup.appendChild(parentItem);
            parentGroup.appendChild(subList);
...
function buildHistoryView(product) {
    ...
    return { rows: actual, premium: true, lockedCount: 0 };
}
