...
def run_scrapers():
    ...
    # Read Web dataset
    web_file = os.path.join(dir_path, "shwapno_products.json")
    web_products = {}
    if os.path.exists(web_file):
        try:
            with open(web_file, "r", encoding="utf-8") as f:
                w_data = json.load(f)
                for p in w_data if isinstance(w_data, list) else w_data.get("products", []):
                    name_key = re.sub(r'\W+', '', p.get("name", "")).lower()
                    if not name_key: continue
                    web_products[name_key] = p
            web_count = len(web_products)
        except Exception as e:
            print(f"[Shwapno] Read web_file error: {e}")
    ...
    for k in all_keys:
        ...
        if w_p and a_p:
            w_price = float(w_p.get("price") or w_p.get("current_price") or 0)
            a_price = float(a_p.get("price") or a_p.get("current_price") or 0)
