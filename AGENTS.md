...
- **GitHub Push Bandwidth Quota Tracker & 1GB postBuffer Push Resilience (`scratch.py`, 2026-09-03, do not regress)**:
  - Added 10GB monthly bandwidth tracking ledger with automatic Dhaka month rollover, push delta byte calculation from git stderr/tree, and rich Telegram telemetry (Push MB, Monthly Consumed, % Used, Remaining GB out of 10.0GB cap).
  - Tuned Git network buffers globally and per-repo (`http.postBuffer = 1048576000` (1GB), `http.version = HTTP/1.1`, `http.lowSpeedLimit = 0`, `http.lowSpeedTime = 999999`, `core.compression = 9`, `pack.windowMemory = 256m`, `pack.packSizeLimit = 2g`) and exponential backoff retry to completely eliminate HTTP 500 RPC push disconnects.
  - Multi-tier secret resolution seamlessly caches credentials into `_PERSISTED_SECRETS` and bakes them into persisted cell headers for 1-time setup cron stability.
- **Aggregator /tmp/* Purge Fix & Multi-Tier p14 Persistence (`aggregator.py` & `scratch.py`, 2026-09-02, do not regress)**:
  - Removed `/tmp/*` wipe from `aggregator.py` (`clean_disk_space`), which was inadvertently deleting IPC result JSONs and logs of early-finishing sub-repos (FooDIE Rest, FoodPANDA, FooDIE Mart, Shwapno, Othoba, CARTup) while `p1` ran concurrently.
  - Multi-tier result persistence: results are now mirrored across `/tmp/p14_results/`, `/tmp/`, and `/kaggle/working/output/p14_results/`.
- **AST-Validated Inline stdout Reconfigure Auto-Patcher (`run_scheduled_repo`, 2026-09-02, do not regress)**:
  - Replaced destructive multi-line try-block regex patching of `TextIOWrapper` and `codecs.getwriter` with inline `getattr(sys.stdout, "reconfigure", lambda **kw: None)(encoding="utf-8", errors="replace")` expressions. Completely prevents `try: try:` syntax errors (such as in PICAboo) and indentation corruption.
  - Added strict `ast.parse()` pre-write validation across all auto-patches (stdout/stderr wrappers, Chaldal fetch_init fallback, Cookups categories fallback, and Cat NoneType safety).
- **Self-Reboot IndentationError Fix & AST Validation Guard (`trigger_self_restart`, 2026-09-02, do not regress)**:
  - Fixed `IndentationError` caused by unindented in-place regex replacement of `_PERSISTED_SECRETS` and `_PERSISTED_STATE`. Strict column-0 matching (`^_PERSISTED_...`) prevents shadowing or breaking nested blocks.
  - Added pre-push `ast.parse()` guard that guarantees generated notebook code is 100% syntactically valid before Kaggle container restart.
- **Sub-Repo Sequential Execution & Directory Isolation (`run_all_scheduled_repos`, 2026-09-02, do not regress)**:
  - Replaced multi-threading with sequential execution and explicit `cwd` parameter on all git and scraper subprocesses. Completely eliminates `os.chdir()` race conditions between worker threads that clobbered repositories and caused 'No execution log or result found'.
- **Python 3.12 TextIOWrapper Auto-Patcher & API Network Fallbacks (`run_scheduled_repo`, 2026-09-02, do not regress)**:
  - Automatically replaces destructive `io.TextIOWrapper(sys.stdout.buffer)` with `sys.stdout.reconfigure()` across all cloned repos (fixing `ValueError: I/O operation on closed file` in Pickaboo).
  - Auto-patches Chaldal `fetch_init` with cached metadata fallback (bypassing `FetchInitDataForCombinedStore` HTTP 500) and Cookups `fetch_categories` with 30s timeout and SQLite cache fallback.
- **Telegram Reporting Protocol (2026-09-01, do not regress)**:
  1. `📊 <b>GroceryGOD Market Summary</b>`: Formatted rich HTML summary generated by `aggregator.py` and sent by `run_grocery_god` after push, containing 1-look monospace `<pre>` table for the 8 in-repo stores, stock deltas (restocked / went OOS), history date range, and live URL (`https://ranehal.github.io/GroceryGOD`).
  2. `📊 <b>Scheduled Sub-Repos (p3–p14) Summary</b>`: Dedicated formatted Telegram summary sent by `_send_p14_summary` when scheduled sub-repos finish, containing a 1-look monospace `<pre>` table for all 12 scheduled repos (FooDIE Rest, FoodPANDA, FooDIE Mart, Shwapno, Othoba, CARTup, Chaldal, COOKup, PICAboo, DARAZ, Meena Bazar, ShareDeal), price change counts (▲/▼/New/OOS), execution durations, stock metrics, delta movements, and direct live dashboard URLs.
  3. `🔄 <b>Kaggle Self-Reboot Telemetry</b>`: Container self-restart dispatch sent by `trigger_self_restart()`, containing exact Dhaka timestamp, reboot count `#{N}`, session duration, pipeline completion badges (GroceryGOD p1, gitw p2, Scheduled Repos p3), and Parquet backup status.
  - **All other noise is SILENCED**: No individual scraper push messages, no 11h timeout warnings, no separate raw log sends.
- **Kaggle Self-Reboot Multi-Tier Resilience & State Telemetry (`trigger_self_restart`, 2026-09-01, do not regress)**:
  - Multi-tier credential fallback: checks `kaggle_secrets.UserSecretsClient`, `os.environ`, `_PERSISTED_SECRETS`, `~/.kaggle/kaggle.json`, and local vault files (`secrets_vault.json`).
  - Cross-restart state persistence: tracks `reboot_count`, `first_boot_dhaka`, `last_reboot_dhaka`, and total runs in `_PERSISTED_STATE` with dual JSON and base64 injection into code cells and local disk vaults (`/kaggle/working/orchestrator_state.json`).
  - Robust metadata auto-recovery: auto-sanitizes `kernel-metadata.json` (cleans dataset/kernel/model sources to strict string lists) and constructs fallback payload directly if `kernels_pull` fails or live notebook source is missing.
  - Captures master loop execution stats (`elapsed_seconds`, `p1_status`, `p2_status`, `p3_status`, parquet backup status) and dispatches comprehensive Telegram summary.
  - Subprocess timeout protection: `run_gitw()` enforces 30-min per-task timeouts to prevent background worker deadlocks from blocking container restart.
