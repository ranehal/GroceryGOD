...
def run_scheduled_repo(repo_url, script_name, label, github_pat):
    ...
    try:
        ...
        for _attempt in range(1, max_script_retries + 1):
            ...
            try:
                with open(_err_log, "w", encoding="utf-8") as _ef:
                    _ef.write("".join(stderr_capture) + "\n--- STDOUT TAIL ---\n" + "\n".join(list(stdout_last_lines)[-30:]))
            except Exception:
                pass
            safe_err = html.escape("".join(stderr_capture)[:5000])
        ...
        tg_send(f"✅ <b>{label}</b> — Data Pushed to GitHub successfully.")
    except Exception as e:
        safe_tb = html.escape(traceback.format_exc()[-500:])
